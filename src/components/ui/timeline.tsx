...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-transparent font-sans container mx-auto"
      ref={containerRef}
    >
...
const TimelineItem = ({ item, index }: { item: TimelineEntry; index: number }) => {
  ...
  const { scrollYProgress } = useScroll({
    target: itemRef,
    offset: ["start 10%", "start 5%"],
  });

  const dotBorderColor = useTransform(
    scrollYProgress,
    [0, 0.1],
    ["var(--neutral-700)", "#22c55e"] // Green border
  );

  const dotBgColor = useTransform(
    scrollYProgress,
    [0, 0.1],
    ["var(--neutral-800)", "#166534"] // Dark green bg
  );

  return (
    <div
      ref={itemRef}
      className={`flex justify-start pt-10 ${index === 0 ? "md:pt-16" : "md:pt-40"} md:gap-10`}
    >
