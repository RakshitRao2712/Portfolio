...
const CONFIG = {
  ...
  description:
    "Automatically categorize and search through my diverse skillsets and project phases with contextual awareness.",
      ...
      <div className="container">
        <div className="section-header reveal">
          ...
          <p className="section-subtitle">
            A detailed mapping of my software skills, development frameworks, databases, and DevOps workflows.
          </p>
        </div>

        <div className="flex items-center justify-center p-4 sm:p-6 w-full not-prose reveal reveal-delay-1">
          <div className="group relative w-full max-w-[500px] mx-auto overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 bg-[#1a1a1c] p-3 sm:p-4 shadow-2xl transition-all duration-500 hover:-translate-y-1">
            <div
              ref={containerRef}
              className={cn(
                "relative w-full overflow-hidden rounded-[1.4rem] sm:rounded-[1.6rem] bg-[#111113]",
            ...
            >
                  ...
                  {TAG_ROWS.map((row, rowIndex) => (
                    ...
                    >
                      {[...row, ...row, ...row].map((item, idx) => (
                        <div
                          key={`${item.id}-${idx}`}
                          className="flex gap-2 bg-black/50 backdrop-blur-sm whitespace-nowrap w-fit text-white/50 p-2 px-3 items-center border border-white/10 rounded-full text-xs"
                        >
                    ...
                    >
                      {[...row, ...row, ...row].map((item, idx) => (
                        <div
                          ...
                          className="flex gap-2 bg-[#0f0f11] whitespace-nowrap w-fit text-white p-2 px-3 items-center border border-white/25 shadow-sm rounded-full text-xs scale-125 ml-6"
                        >
                          <HugeiconsIcon icon={item.icon} size={14} className="text-white" />
              ...
              {/* edge fades */}
              <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-[#111113] to-transparent z-20" />
              <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-[#111113] to-transparent z-20" />
            </div>
